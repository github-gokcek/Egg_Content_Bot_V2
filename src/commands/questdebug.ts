...
module.exports = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { 
          ...
          value: 
            `**Mesaj Sayısı:** ${userQuests.messageCount}\n` +
        ...
        { 
          ...
          value: 
            `**Toplam:** ${userQuests.reactionGivenToUsers?.size || 0} farklı kullanıcı\n` +
    ...
    const activeQuests = userQuests.quests.map(q => 
      `${q.emoji} **${q.name}**: ${q.progress}/${q.target} ${q.completed ? '✅' : ''}`
    ).join('\n');
