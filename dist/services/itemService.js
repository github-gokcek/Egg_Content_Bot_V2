...
class ItemService {
    // Use consumable item
    static async useItem(character, itemId) {
        ...
        const stats = rpgService_1.rpgService.calculateDerivedStats(character);
        let effectMessage = '';
        // Apply item effects
        switch (itemId) {
            case 'health_potion':
        ...
        logger_1.Logger.success('Item used', { userId: character.userId, itemId });
        return {
            success: true,
    ...
    // Use item in combat
    static async useItemInCombat(character, itemId) {
        const item = (0, items_1.getItem)(itemId);
        if (!item) {
            return { success: false, message: '❌ Item bulunamadı!' };
        }
