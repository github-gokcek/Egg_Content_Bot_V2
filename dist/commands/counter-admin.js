...
const counterAnalysisService_1 = require("../services/counterAnalysisService");
    ...
    async execute(interaction) {
        ...
        try {
            if (subcommand === 'analyze-player') {
                ...
                const matchupsAnalyzed = await counterAnalysisService_1.counterAnalysisService.analyzePlayerMatches(riotId, matchCount);
                await interaction.editReply(`✅ **Analiz Tamamlandı!**\n` +
                    `👤 Oyuncu: ${riotId}\n` +
                ...
                const matchCount = interaction.options.getInteger('match-count') || 20;
                await interaction.editReply(`🔄 **${riotIds.length} oyuncu analiz ediliyor...**\n` +
                    `Bu işlem birkaç dakika sürebilir...`);
                const totalMatches = await counterAnalysisService_1.counterAnalysisService.analyzeMultiplePlayers(riotIds, matchCount);
                await interaction.editReply(`✅ **Toplu Analiz Tamamlandı!**\n` +
                    `👥 Oyuncu sayısı: ${riotIds.length}\n` +
                    `📊 Toplam matchup: ${totalMatches}\n` +
                    `💾 Veritabanı güncellendi!`);
            }
            else if (subcommand === 'stats') {
                const stats = counterAnalysisService_1.counterAnalysisService.getStats();
                await interaction.reply({
                    content: `📊 **Counter Veritabanı İstatistikleri**\n\n` +
                ...
                const data = counterAnalysisService_1.counterAnalysisService.getCounters(champion);
                if (!data) {
                    return interaction.reply({
            ...
            logger_1.Logger.error('[Counter Admin] Hata:', error);
