...
import { getShopItems, getItem } from '../data/items';
...
export async function handleShop(interaction: ChatInputCommandInteraction) {
  ...
  if (weapons.length > 0) {
    const weaponList = weapons.map(item => 
      `\`${item.id}\` - **${item.name}** (Lv.${item.level}) - ${item.price} 🪙`
    ).join('\n');
  ...
  if (accessories.length > 0) {
    const accList = accessories.map(item => 
      `\`${item.id}\` - **${item.name}** (Lv.${item.level}) - ${item.price} 🪙`
    ).join('\n');
  ...
  if (consumables.length > 0) {
    const consList = consumables.map(item => 
...
export async function handleBuy(interaction: ChatInputCommandInteraction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: '💰 Harcanan', value: `${totalPrice} 🪙`, inline: true },
      { name: '💰 Kalan', value: `${character.rpgCoin} 🪙`, inline: true }
    )
    .setTimestamp();
...
export async function handleSell(interaction: ChatInputCommandInteraction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: '💰 Kazanılan', value: `${totalSellPrice} 🪙`, inline: true },
      { name: '💰 Toplam', value: `${character.rpgCoin} 🪙`, inline: true }
    )
    .setTimestamp();
