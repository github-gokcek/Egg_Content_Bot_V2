...
module.exports = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .setRequired(true))
        .addStringOption(option => option.setName('islem')
        .setDescription('Yapılacak işlem')
        .setRequired(true)
        .addChoices({ name: '➕ Bakiye Ekle', value: 'add' }, { name: '➖ Bakiye Çıkar', value: 'remove' }, { name: '🔄 Bakiye Ayarla', value: 'set' }))
    ...
    async execute(interaction) {
        ...
        let player = await databaseService_1.databaseService.getPlayer(targetUser.id);
        if (!player) {
            return interaction.reply({
        ...
        const embed = new discord_js_1.EmbedBuilder()
            .setColor(operationColor[operation])
            .setTitle('💰 Bakiye Yönetimi')
            .setDescription(`**${targetUser.username}** kullanıcısının bakiyesi güncellendi!`)
            .addFields({ name: '👤 Kullanıcı', value: `<@${targetUser.id}>`, inline: true }, { name: '⚙️ İşlem', value: operationText[operation], inline: true }, { name: '💎 Miktar', value: `${amount} 🪙`, inline: true }, { name: '📊 Eski Bakiye', value: `${oldBalance} 🪙`, inline: true }, { name: '📈 Yeni Bakiye', value: `${newBalance} 🪙`, inline: true }, { name: '📝 Değişim', value: `${newBalance - oldBalance > 0 ? '+' : ''}${newBalance - oldBalance} 🪙`, inline: true })
            .setThumbnail(targetUser.displayAvatarURL())
            .setFooter({ text: `İşlemi yapan: ${interaction.user.username}` })
            .setTimestamp();
        logger_1.Logger.info('Bakiye yönetimi', {
