...
exports.championDataService = exports.ChampionDataService = void 0;
const axios_1 = __importDefault(require("axios"));
const logger_1 = require("../utils/logger");
const championMetaData_1 = require("../data/championMetaData");
...
class ChampionDataService {
    ...
    async loadChampions() {
        if (this.championCache.size > 0)
            return;
        try {
            const response = await axios_1.default.get(`${DDRAGON_BASE}/data/tr_TR/champion.json`);
            ...
            logger_1.Logger.success(`${this.championCache.size} şampiyon yüklendi`);
        }
        catch (error) {
            logger_1.Logger.error('Şampiyon verisi yüklenemedi:', error);
    ...
    async getBuild(championName, role = 'jungle') {
        ...
        try {
            ...
            if (!response.data || !Array.isArray(response.data) || response.data.length === 0) {
                logger_1.Logger.warn(`U.GG API'den build verisi alınamadı, fallback kullanılıyor: ${champion.id}`);
            ...
            logger_1.Logger.error('Build verisi alınamadı:', error.response?.status || error.message);
            return this.getFallbackBuild(champion.id);
        }
    }
    async getCounters(championName) {
        const champion = this.findChampion(championName);
        if (!champion) {
            throw new Error(`Şampiyon bulunamadı: ${championName}`);
        }
        // Statik counter verilerini kullan
        logger_1.Logger.info(`[COUNTER] Şampiyon: ${champion.name} - Statik veri kullanılıyor`);
        return this.getStaticCounters(champion.id);
    }
    async getMatchup(champion1Name, champion2Name) {
        const champ1 = this.findChampion(champion1Name);
        const champ2 = this.findChampion(champion2Name);
        if (!champ1)
            throw new Error(`Şampiyon bulunamadı: ${champion1Name}`);
        if (!champ2)
            throw new Error(`Şampiyon bulunamadı: ${champion2Name}`);
        logger_1.Logger.info(`[MATCHUP] ${champ1.name} vs ${champ2.name} - Statik veri kullanılıyor`);
    ...
    getRecommendedBuild(championId) {
        return championMetaData_1.championBuilds[championId] || { items: [6630, 3047, 3071, 6333, 3053, 3065], core: ['Mythic Item', 'Boots', 'Legendary Items'], rune: 'Conqueror' };
    }
    getRecommendedRunes(championId) {
        const build = championMetaData_1.championBuilds[championId];
        if (build && build.rune) {
            return { primary: 8010, secondary: 8000, name: build.rune };
        }
        return { primary: 8010, secondary: 8000, name: 'Conqueror' };
    }
    getStaticCounters(championId) {
        // championCounterData'dan al
        if (championMetaData_1.championCounterData[championId]) {
            return championMetaData_1.championCounterData[championId];
        }
...
exports.championDataService = new ChampionDataService();
