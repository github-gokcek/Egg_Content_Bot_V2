import { Enemy, LootDrop } from '../types/rpg';
...
export class EnemyGenerator {
  ...
  static generateEnemy(playerLevel: number, areaKey?: string): Enemy {
    // Determine area
    let area = AREA_ENEMIES.forest;
    if (areaKey && AREA_ENEMIES[areaKey]) {
      area = AREA_ENEMIES[areaKey];
    } else {
      // Auto-select area based on player level
      for (const [key, areaData] of Object.entries(AREA_ENEMIES)) {
  ...
  static generateBoss(playerLevel: number, areaKey?: string): Enemy {
  ...
  static getEnemyImage(enemyName: string): string {
    for (const [key, template] of Object.entries(ENEMY_TEMPLATES)) {
  ...
  // Get enemy emoji
  static getEnemyEmoji(enemyName: string): string {
    for (const [key, template] of Object.entries(ENEMY_TEMPLATES)) {
      if (enemyName.includes(template.name)) {
