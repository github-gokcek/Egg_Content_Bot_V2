...
const riotApiService_1 = require("../services/riotApiService");
const championDataService_1 = require("../services/championDataService");
...
module.exports = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('oyuncu')
        ...
        .addStringOption(option => option.setName('sampiyon')
        ...
        .addStringOption(option => option.setName('sampiyon1')
        .setDescription('İlk şampiyon (Örnek: Renekton)')
        .setRequired(true))
        .addStringOption(option => option.setName('sampiyon2')
    ...
    async execute(interaction) {
        ...
        try {
            if (['build', 'counter', 'matchup'].includes(subcommand)) {
                await championDataService_1.championDataService.loadChampions();
            }
            if (subcommand === 'build') {
                ...
                const summoner = await riotApiService_1.riotApiService.getSummonerByPuuid(account.puuid);
            ...
            logger_1.Logger.error('LoL komut hatası:', error);
            await interaction.editReply({ content: `❌ ${error.message || 'Bir hata oluştu!'}` });
        }
    },
};
async function handleProfile(interaction, summoner, account) {
    try {
        // Ranked verilerini al
        let rankedData = [];
        try {
            rankedData = await riotApiService_1.riotApiService.getRankedData(summoner.id);
        }
        catch (error) {
            logger_1.Logger.warn('Ranked data alınamadı, devam ediliyor...');
        }
        ...
        let totalGames = 0;
        try {
            const matchIds = await riotApiService_1.riotApiService.getMatchHistory(account.puuid, 20);
            const matches = await Promise.all(matchIds.slice(0, 20).map(id => riotApiService_1.riotApiService.getMatchDetails(id)));
            ...
            logger_1.Logger.warn('Maç geçmişi alınamadı, devam ediliyor...');
        }
        // Champion mastery al
        let topChampions = [];
        try {
            const masteries = await riotApiService_1.riotApiService.getChampionMastery(account.puuid, 3);
            await championDataService_1.championDataService.loadChampions();
            for (const mastery of masteries) {
                const championName = championDataService_1.championDataService.getChampionById(mastery.championId);
                if (championName) {
                    topChampions.push({
                        name: championName,
            ...
            logger_1.Logger.warn('Champion mastery alınamadı, devam ediliyor...');
        }
        // Embed oluştur
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: '🎯 Seviye', value: `${summoner.summonerLevel}`, inline: true });
        ...
        logger_1.Logger.error('Profil verisi alınırken hata:', error);
        await interaction.editReply({ content: `❌ Profil bilgileri alınamadı: ${error.message}` });
    }
}
async function handleMatchHistory(interaction, puuid, summoner, count) {
    const matchIds = await riotApiService_1.riotApiService.getMatchHistory(puuid, count);
    ...
    await interaction.editReply({ embeds: [embed] });
    const matches = await Promise.all(matchIds.slice(0, 5).map(id => riotApiService_1.riotApiService.getMatchDetails(id)));
    let description = '';
    for (const match of matches) {
...
async function handleLiveGame(interaction, puuid, summoner) {
    try {
        const liveGame = await riotApiService_1.riotApiService.getCurrentGame(puuid);
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: '🎮 Oyun Modu', value: liveGame.gameMode, inline: true }, { name: '⏱️ Süre', value: `${gameDuration} dakika`, inline: true }, { name: '🦸 Şampiyon', value: participant?.championId ? `Champion ID: ${participant.championId}` : 'Bilinmiyor', inline: true })
            .setTimestamp();
        await interaction.editReply({ embeds: [embed] });
    }
    catch (error) {
...
async function handleMastery(interaction, puuid, summoner, account) {
    const masteries = await riotApiService_1.riotApiService.getChampionMastery(puuid, 5);
    ...
    for (let i = 0; i < masteries.length; i++) {
        ...
        const championName = championDataService_1.championDataService.getChampionById(mastery.championId) || `Champion ${mastery.championId}`;
...
async function handleCounter(interaction, championName) {
    try {
        ...
        // Ekran görüntüsü çek
        try {
            const screenshotPath = await screenshotService_1.screenshotService.captureCounterPage(championId);
            if (screenshotPath) {
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setTitle(`⚔️ ${champion.name} Counterları`)
            .setThumbnail(`${DDRAGON_BASE}/img/champion/${champion.id}.png`)
            .setDescription(`**${champion.name}** için counter bilgilerini görmek için:\n\n` +
        ...
        logger_1.Logger.error('Counter komut hatası:', error);
        try {
            await interaction.editReply({ content: `❌ ${error.message}` });
        }
        catch (e) {
...
async function handleMatchup(interaction, champ1Name, champ2Name) {
    try {
        const champion1 = championDataService_1.championDataService.findChampion(champ1Name);
        const champion2 = championDataService_1.championDataService.findChampion(champ2Name);
        if (!champion1) {
        ...
        try {
            const screenshotPath = await screenshotService_1.screenshotService.captureMatchupPage(championId1, championId2);
            if (screenshotPath) {
                const attachment = new discord_js_1.AttachmentBuilder(screenshotPath);
                const embed = new discord_js_1.EmbedBuilder()
                    .setColor(0xffa500)
        ...
        const matchup = await championDataService_1.championDataService.getMatchup(champ1Name, champ2Name);
        if (matchup.games === 0) {
            // Veri yoksa sadece U.GG linkini göster
            const embed = new discord_js_1.EmbedBuilder()
                .setColor(0xffa500)
                .setTitle(`⚔️ ${champion1.name} vs ${champion2.name}`)
                .setDescription(`Bu matchup için detaylı bilgi almak için:\n\n` +
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setDescription(`**${matchup.champion1}** win rate: **${winRate}%**\n` +
        ...
        logger_1.Logger.error('Matchup komut hatası:', error);
        try {
            await interaction.editReply({ content: `❌ ${error.message}` });
        }
        catch (e) {
