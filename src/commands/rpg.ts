...
module.exports = {
      ...
      subcommand
        ...
        .addStringOption(option =>
          option
            ...
            .setRequired(false)
            .addChoices(
              { name: '🌲 Peaceful Forest (Lv.1-5)', value: 'forest' },
              { name: '🌑 Dark Forest (Lv.5-10)', value: 'dark_forest' },
              { name: '👻 Haunted Woods (Lv.10-15)', value: 'haunted_woods' },
              { name: '☁️ Sky Realm (Lv.15-25)', value: 'sky_realm' }
            )
        )
    )
    .addSubcommand(subcommand =>
      subcommand
        ...
        .addStringOption(option =>
          option
            ...
            .setRequired(true)
            .addChoices(
              { name: 'Server Coin → RPG Coin', value: 'server_to_rpg' },
              { name: 'RPG Coin → Server Coin', value: 'rpg_to_server' }
            )
        )
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subcommand === 'create') {
      ...
      const { handleShop } = await import('../services/shopService');
      await handleShop(interaction);
    } else if (subcommand === 'buy') {
      const { handleBuy } = await import('../services/shopService');
      await handleBuy(interaction);
    } else if (subcommand === 'sell') {
      const { handleSell } = await import('../services/shopService');
  ...
  async handleCreate(interaction: ChatInputCommandInteraction) {
    ...
    const existingCharacter = await rpgService.getCharacter(interaction.user.id);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        Object.entries(CLASS_INFO).map(([classKey, info]) => ({
          name: info.name,
  ...
  async handleProfile(interaction: ChatInputCommandInteraction) {
    ...
    const character = await rpgService.getCharacter(targetUser.id);
    if (!character) {
      return interaction.reply({
    ...
    const derivedStats = rpgService.calculateDerivedStats(character);
      ...
      .addFields(
        {
          name: '📊 Stats',
          value: 
            `**HP:** ${character.currentHp}/${derivedStats.maxHp}\n` +
        ...
        },
        {
          name: '💎 Base Stats',
          value:
            `STR: ${character.stats.str} | DEX: ${character.stats.dex}\n` +
  ...
  async handleExchange(interaction: ChatInputCommandInteraction) {
    const type = interaction.options.getString('tip', true);
    const amount = interaction.options.getInteger('miktar', true);

    await interaction.deferReply({ flags: 64 });

    const rates = await rpgService.getExchangeRates();
    ...
    if (type === 'server_to_rpg') {
      ...
      const rpgCoinAmount = await rpgService.exchangeServerToRPG(interaction.user.id, amount);
      ...
      const serverCoinAmount = await rpgService.exchangeRPGToServer(interaction.user.id, amount);
  ...
  async handleAdventure(interaction: ChatInputCommandInteraction) {
    ...
    const { EnemyGenerator } = await import('../services/enemyService');
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '🎯 Your Stats', value: `ATK: ${characterStats.attack} | DEF: ${characterStats.defense} | SPD: ${characterStats.speed}`, inline: false },
        { name: '📜 Combat Log', value: '> Combat started! Choose your action.', inline: false }
      )
    ...
    try {
      const imagePath = join(process.cwd(), 'assets', 'enemies', enemyImage);
  ...
  async handleRaid(interaction: ChatInputCommandInteraction) {
    ...
    const memberCharacters = await Promise.all(
      group.members.map(async (memberId) => {
        const char = await rpgService.getCharacter(memberId);
    ...
    const participantStats = memberCharacters.map(m => ({
      userId: m.userId,
      speed: rpgService.calculateDerivedStats(m.character!).speed
    }));
    const turnOrder = RaidService.calculateTurnOrder(participantStats);

    // Initialize participant states
    const participantStates: any = {};
    for (const member of memberCharacters) {
      const stats = rpgService.calculateDerivedStats(member.character!);
    ...
    const { EnemyGenerator } = await import('../services/enemyService');
    const { CombatService } = await import('../services/combatService');
    const { BossTraitHandler } = await import('../services/bossTraitHandler');
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '📜 Combat Log', value: '> Raid combat started! Work together to defeat 3 bosses!', inline: false },
        { name: '🎯 Current Turn', value: `<@${turnOrder[0]}> - Choose your action!`, inline: false }
      )
  ...
  async handleInventory(interaction: ChatInputCommandInteraction) {
    const targetUser = interaction.options.getUser('kullanici') || interaction.user;
    ...
    if (character.equipment.weapon) equippedList.push(`⚔️ Weapon: ${character.equipment.weapon}`);
    if (character.equipment.armor) equippedList.push(`🛡️ Armor: ${character.equipment.armor}`);
    if (character.equipment.accessory) equippedList.push(`💍 Accessory: ${character.equipment.accessory}`);
    ...
    embed.addFields(
      { name: '💰 RPG Coin', value: `${character.rpgCoin} 🪙`, inline: true },
      { name: '📊 Capacity', value: `${character.inventory.length}/${character.maxInventorySize}`, inline: true }
    );

    await interaction.reply({ embeds: [embed] });
  },

  async handleEquip(interaction: ChatInputCommandInteraction) {
  ...
  async handleUnequip(interaction: ChatInputCommandInteraction) {
    const slot = interaction.options.getString('slot', true) as 'weapon' | 'armor' | 'accessory';
  ...
  async handleUse(interaction: ChatInputCommandInteraction) {
    const itemId = interaction.options.getString('item', true);
  ...
  async handleSkills(interaction: ChatInputCommandInteraction) {
    const character = await rpgService.getCharacter(interaction.user.id);
    if (!character) {
      return interaction.reply({
    ...
    const { getAvailableSkills } = await import('../data/skills');
    ...
    if (availableSkills.length === 0) {
      ...
      const skillList = availableSkills.map(skill => 
        `**${skill.name}** (Lv.${skill.levelRequired})\n` +
        `> ${skill.description}\n` +
        `> 💧 Mana: ${skill.manaCost} | ⏱️ Cooldown: ${skill.cooldown} turn`
      ).join('\n\n');
  ...
  async handleRest(interaction: ChatInputCommandInteraction) {
    const character = await rpgService.getCharacter(interaction.user.id);
    if (!character) {
      return interaction.reply({
        content: '❌ Önce `/rpg create` ile karakter oluşturmalısın!',
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${character.name}** biraz dinlendi ve enerjisini toparladı.\n\n` +
  ...
  async handleFull(interaction: ChatInputCommandInteraction) {
    // Admin check
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${character.name}** tam can ve mana ile dolduruldu!\n\n` +
