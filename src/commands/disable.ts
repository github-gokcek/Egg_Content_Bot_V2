...
module.exports = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subcommand === 'game_channel') {
      const disabled = await configService.disableGameChannel(interaction.guildId, game);
      if (disabled) {
      ...
      const disabled = await configService.disableWinnerLogChannel(interaction.guildId, game);
      if (disabled) {
        await interaction.reply({ 
