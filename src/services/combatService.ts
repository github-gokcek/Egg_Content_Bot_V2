import { RPGCharacter, Enemy, CombatResult, DerivedStats } from '../types/rpg';
...
export class CombatService {
  ...
  // Check dodge
  static checkDodge(dodgeChance: number): boolean {
  ...
  static async processCombatResult(
    character: RPGCharacter,
    enemy: Enemy,
    victory: boolean
    ...
    const result: CombatResult = {
      ...
      damageTaken: rpgService.calculateDerivedStats(character).maxHp - character.currentHp,
      xpGained: 0,
      coinGained: 0,
      itemsLooted: [],
      leveledUp: false
    ...
    if (victory) {
      ...
      const levelUpResult = await rpgService.addXP(character, enemy.xpReward);
  ...
  static calculateFleeChance(characterSpeed: number, enemySpeed: number): number {
  ...
  // Attempt to flee
  static attemptFlee(characterSpeed: number, enemySpeed: number): boolean {
  ...
  static generateCombatLog(turn: CombatTurn, turnNumber?: number): string {
    let log = '';
    ...
    if (turn.playerAction === 'attack') {
      if (turn.isDodged) {
        ...
        if (turn.isCrit) log += ' **CRITICAL HIT!**';
        log += '\n';
      }
  ...
  // Create HP bar
  static createHPBar(current: number, max: number, length: number = 10): string {
