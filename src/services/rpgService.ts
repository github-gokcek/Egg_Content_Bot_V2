...
// Class Multipliers
const CLASS_MULTIPLIERS: Record<RPGClass, ClassMultipliers> = {
  [RPGClass.WARRIOR]: { str: 1.4, dex: 0.9, int: 0.6, vit: 1.3, agi: 0.8 },
  [RPGClass.MAGE]: { str: 0.6, dex: 0.9, int: 1.6, vit: 0.8, agi: 1.1 },
  [RPGClass.ARCHER]: { str: 1.1, dex: 1.3, int: 0.7, vit: 0.9, agi: 1.4 },
  [RPGClass.ASSASSIN]: { str: 1.2, dex: 1.5, int: 0.7, vit: 0.7, agi: 1.6 },
  [RPGClass.CLERIC]: { str: 0.8, dex: 1.0, int: 1.3, vit: 1.2, agi: 0.9 }
};
...
class RPGService {
  ...
  calculateDerivedStats(character: RPGCharacter): DerivedStats {
  ...
  async createCharacter(
    ...
    await setDoc(doc(db, 'rpgCharacters', userId), {
      ...character,
    ...
    Logger.success('RPG Character created', { userId, name, class: characterClass });
    return character;
  }

  // Get character
  async getCharacter(userId: string): Promise<RPGCharacter | null> {
    try {
      const docSnap = await getDoc(doc(db, 'rpgCharacters', userId));
      ...
      Logger.error('getCharacter error', error);
      return null;
    }
  }

  // Update character
  async updateCharacter(character: RPGCharacter): Promise<void> {
    try {
      await updateDoc(doc(db, 'rpgCharacters', character.userId), {
        ...character,
        createdAt: character.createdAt.toISOString(),
      ...
      Logger.success('RPG Character updated', { userId: character.userId });
    } catch (error) {
  ...
  async addXP(character: RPGCharacter, xp: number): Promise<{ leveledUp: boolean; newLevel?: number }> {
    character.xp += xp;
    let leveledUp = false;
    ...
    while (character.xp >= character.xpToNextLevel) {
      ...
      Logger.success('Character leveled up', { userId: character.userId, newLevel });
    }
  ...
  async calculateExchangeRates(): Promise<EconomyRate> {
    try {
      // Get all players
      ...
      const charactersSnap = await getDocs(collection(db, 'rpgCharacters'));
      let totalRPGCoin = 0;
      ...
      if (totalServerCoin === 0) totalServerCoin = 1;
      if (totalRPGCoin === 0) totalRPGCoin = 1;
      ...
      // Save to database
      await setDoc(doc(db, 'rpgEconomy', 'rates'), {
        ...rate,
      ...
      Logger.error('calculateExchangeRates error', error);
  ...
  async getExchangeRates(): Promise<EconomyRate> {
    try {
      const docSnap = await getDoc(doc(db, 'rpgEconomy', 'rates'));
      if (!docSnap.exists()) {
  ...
  async exchangeServerToRPG(userId: string, serverCoinAmount: number): Promise<number> {
  ...
  async exchangeRPGToServer(userId: string, rpgCoinAmount: number): Promise<number> {
    const rates = await this.getExchangeRates();
