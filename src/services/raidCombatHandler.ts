...
import { RaidCombat } from '../types/rpg';
import { EnemyGenerator } from './enemyService';
import { BossTraitHandler } from './bossTraitHandler';
import { getBossTrait } from '../data/bossTraits';

export async function handleRaidCombatButton(interaction: ButtonInteraction) {
  ...
  if (actionType === 'attack') {
    ...
    if (currentBoss.hp <= 0) {
      ...
      if (raid.currentBossIndex >= raid.bosses.length) {
        // RAID COMPLETE!
        await deleteDoc(doc(db, 'activeRaids', raidId));
        
        // Raid sayısını artır (tüm katılımcılar için)
        for (const userId of raid.participants) {
          const char = await rpgService.getCharacter(userId);
          if (char) {
            ...
            await rpgService.updateCharacter(char);
          }
        }
        ...
        const victoryEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: '🏆 Final Boss', value: currentBoss.name, inline: true },
            { name: '⚔️ Total Turns', value: raid.turnCount.toString(), inline: true },
            { name: '👥 Survivors', value: Object.values(raid.participantStates).filter(s => s.hp > 0).length.toString(), inline: true }
          )
          .setTimestamp();
...
async function updateRaidEmbed(interaction: ButtonInteraction, raid: RaidCombat) {
  const currentBoss = raid.bosses[raid.currentBossIndex];
  const enemyEmoji = EnemyGenerator.getEnemyEmoji(currentBoss.name);
  const enemyHpBar = CombatService.createHPBar(currentBoss.hp, currentBoss.maxHp, 15);

  // Get all participant HP bars
  const participantInfo = await Promise.all(
    raid.participants.map(async (userId) => {
      const char = await rpgService.getCharacter(userId);
      if (!char) return null;
      
      const stats = rpgService.calculateDerivedStats(char);
      const state = raid.participantStates[userId];
      const hp = Math.max(0, state.hp);
      const hpBar = CombatService.createHPBar(hp, stats.maxHp, 10);
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: '📜 Combat Log', value: recentLogs || 'Combat started!', inline: false },
      { name: '🎯 Current Turn', value: `<@${raid.currentTurnUserId}> - Choose your action!`, inline: false }
    )
...
async function distributeRaidRewards(raid: RaidCombat, finalBoss: any) {
  const embeds: EmbedBuilder[] = [];
  
  for (const userId of raid.participants) {
    const character = await rpgService.getCharacter(userId);
    if (!character) continue;

    // Calculate rewards (each boss gives rewards)
    let totalXP = 0;
    let totalCoins = 0;
    const allLoot: { itemId: string; amount: number }[] = [];
      ...
      for (const loot of boss.lootTable) {
        ...
        if (roll < loot.chance) {
          const amount = Math.floor(
            Math.random() * (loot.maxAmount - loot.minAmount + 1) + loot.minAmount
          );
