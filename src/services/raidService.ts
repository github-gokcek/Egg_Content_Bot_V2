...
import { getRandomBossTrait } from '../data/bossTraits';

export class RaidService {
  // Generate 3 bosses for raid (stronger than normal enemies)
  static generateRaidBosses(averageLevel: number): Enemy[] {
    const bosses: Enemy[] = [];
    
    // Boss 1: Mini Boss (2x stronger)
    const boss1 = EnemyGenerator.generateEnemy(averageLevel, undefined);
    const trait1 = getRandomBossTrait();
    ...
    const boss2 = EnemyGenerator.generateEnemy(averageLevel + 2, undefined);
    const trait2 = getRandomBossTrait();
    ...
    const boss3 = EnemyGenerator.generateEnemy(averageLevel + 5, undefined);
    const trait3 = getRandomBossTrait();
  ...
  static calculateTurnOrder(participants: Array<{ userId: string; speed: number }>): string[] {
    return participants
  ...
  // Get next turn user
  static getNextTurnUser(currentUserId: string, turnOrder: string[]): string {
