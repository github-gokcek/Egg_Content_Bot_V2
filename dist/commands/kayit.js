...
module.exports = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addStringOption(opt => opt.setName('lol_ign')
        .setDescription('League of Legends oyun içi adınız')
        .setRequired(false))
        .addStringOption(opt => opt.setName('tft_ign')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (!player) {
                ...
                if (lolIgn)
                    player.lolIgn = lolIgn;
                if (tftIgn)
                    player.tftIgn = tftIgn;
            }
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .addFields({ name: '🎮 Discord', value: interaction.user.username, inline: true }, { name: '⚔️ LoL IGN', value: lolIgn || '*Belirtilmedi*', inline: true }, { name: '♟️ TFT IGN', value: tftIgn || lolIgn || '*Belirtilmedi*', inline: true }, { name: '💰 Başlangıç Bakiyesi', value: '100 🪙', inline: false })
                .setThumbnail(interaction.user.displayAvatarURL())
                .setTimestamp();
            await interaction.reply({ embeds: [embed] });
            logger_1.Logger.success('Oyuncu kaydedildi', {
                discordId: interaction.user.id,
                lolIgn,
                tftIgn
            });
